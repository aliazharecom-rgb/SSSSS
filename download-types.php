...
switch ($_POST['type'] ?? $_GET['type']) {
    case 'users':
       $jsonarray = json_encode(
        ...
        header('Content-Disposition: attachment; filename="UMAR AUTHUsers.json"');
        header('Expires: 0');
    ...
    case 'licenses':
       $query = misc\mysql\query("SELECT * FROM `keys` WHERE `app` = ?",[$_SESSION['app']]);
        ...
        header('Content-Disposition: attachment; filename="UMAR AUTHKeys.json"');
        header('Expires: 0');
        header('Cache-Control: must-revalidate');
        header('Pragma: public');
    ...
    case 'logs':
        ...
        header('Content-Description: File Transfer');
        header('Content-Type: application/octet-stream');
        header('Content-Disposition: attachment; filename="UMAR AUTHUserLogs.json"');
        header('Expires: 0');
        header('Cache-Control: must-revalidate');
        header('Pragma: public');
        header('Content-Length: ' . strlen($newjson));
    ...
    case 'auditLog':
        ...
        header('Content-Description: File Transfer');
        header('Content-Type: application/octet-stream');
        header('Content-Disposition: attachment; filename="UMAR AUTHAuditLogs.json"');
        header('Expires: 0');
        header('Cache-Control: must-revalidate');
        header('Pragma: public');
        header('Content-Length: ' . strlen($newjson));
